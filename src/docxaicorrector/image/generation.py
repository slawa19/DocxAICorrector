...
from docxaicorrector.image.analysis import (
    ImageBudget,
    encoded_image_within_byte_budget,
    image_within_pixel_budget,
)
...
def generate_image_candidate(
    ...
    *,
    ...
    pixel_budget: ImageBudget | None = None,
) -> bytes:
...
def _admit_generated_image_bytes(
    candidate_bytes: bytes,
    pixel_budget: ImageBudget | None,
    *,
    stage: str,
) -> bool:
    """Gate + charge a provider-returned raster BEFORE any ``.load()`` decode.

    Runs the header-only per-image pixel gate on the GENERATED bytes (round-5
    finding 5b — so ``_restore_*`` never ``.load()``s an over-budget provider
    image) and, when a per-document :class:`ImageBudget` is supplied, charges the
    generated raster against it (finding 5c — a single shared budget across
    embedded + generated). Returns False when the image should be dropped in
    favour of the original source raster.
    """
    if not image_within_pixel_budget(candidate_bytes, stage=stage):
        return False
    if pixel_budget is not None and not pixel_budget.charge(candidate_bytes, stage=stage):
        return False
    return True
...
def _generate_creative_candidate(
    ...
    *,
    ...
    # Gate + charge the provider-returned raster BEFORE _restore_generated_output
    # loads it (round-5 finding 5b/5c); degrade to the original source on reject.
    if not _admit_generated_image_bytes(candidate_bytes, pixel_budget, stage="creative_semantic_generate_output"):
        return image_bytes
...
def _generate_direct_semantic_candidate(
    ...
    *,
    ...
    candidate_bytes, revised_prompt = _extract_image_bytes(response)
    # Gate + charge the provider-returned raster BEFORE _restore_semantic_output
    # loads it (round-5 finding 5b/5c); degrade to the original source on reject.
    if not _admit_generated_image_bytes(candidate_bytes, pixel_budget, stage="semantic_image_edit_output"):
        return image_bytes
...
def _generate_structured_candidate(
    ...
    *,
    ...
    # Gate + charge the provider-returned raster BEFORE _restore_generated_output
    # loads it (round-5 finding 5b/5c); degrade to the original source on reject.
    if not _admit_generated_image_bytes(candidate_bytes, pixel_budget, stage="structured_image_generate_output"):
        return image_bytes
    candidate_bytes = _restore_generated_output(
        candidate_bytes,
        original_size,
...
def _extract_image_bytes(response) -> tuple[bytes, str | None]:
    ...
    # Byte-cap the ENCODED payload BEFORE base64.b64decode (round-5 finding 5a):
    # b64decode would otherwise allocate ~3/4 of the encoded length as raw bytes
    # for an oversized/degenerate provider payload before any pixel-budget gate
    # could run. Reject (never decode) when the encoded payload is too large.
    if not encoded_image_within_byte_budget(image_base64, stage="image_generation_output"):
        raise RuntimeError("Provider вернул base64 image payload сверх byte budget; отклонено без декодирования.")
    try:
