...
Для видимого прогона в VS Code открывайте встроенный WSL-терминал и запускайте:
...
Для запуска через VS Code Task используйте `Tasks: Run Task` -> `Run Full Pytest WSL Visible`.
