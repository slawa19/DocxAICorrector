...
- `processing_runtime.py` теперь является единым normalization boundary для upload payload: он определяет реальный формат входного файла, при необходимости конвертирует legacy `.doc` в рабочий `.docx` и только потом строит token/payload для остальных слоев.
...
- минимальный post-Pandoc pass форматирует только captions, image-placeholder paragraphs и table baseline, не пытаясь воспроизводить broad source paragraph XML.
