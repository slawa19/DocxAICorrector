...
Isolated benchmark project for comparing PDF normalization and structural extraction candidates on `lietaer-pdf-first-20-structure-core` without changing the production app path.
...
```bash
bash benchmark_projects/pdf_candidate_benchmark/run.sh --source-profile-id lietaer-pdf-first-20-structure-core
```