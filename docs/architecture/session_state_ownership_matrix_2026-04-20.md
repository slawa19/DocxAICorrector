...
| `latest_image_mode` | `state.py` | `init_session_state()`, `reset_run_state()`, `apply_processing_start()` | `app.main()` via `get_latest_image_mode()` — верно на 2026-04-20; с PR #57 (2026-08) импорт из UI ушёл, продового читателя у ключа больше нет, сам `get_latest_image_mode()` жив в `runtime/state.py` | `P1a` |
