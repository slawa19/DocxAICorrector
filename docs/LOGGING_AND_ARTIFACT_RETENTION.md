...
- `marker_paragraph_omitted` — `src/docxaicorrector/generation/_generation.py`, WARNING: модель не вернула текст для отдельных абзацев блока (спека 056 E). Раньше это отбрасывало ВЕСЬ блок и подставляло его английский исходник; теперь блок сохраняет остальные абзацы, а эти получают статус `omitted`. Context: `{omitted_paragraph_ids, omitted_paragraph_count, paragraph_count}`.
- `marker_chunk_paragraph_break_collapsed` — `src/docxaicorrector/generation/_generation.py`, INFO: модель разбила единственный абзац блока на несколько; части склеены обратно в один абзац. Эмитится ТОЛЬКО для блока с одним маркером, где присвоение текста доказуемо однозначно. Context: `{paragraph_id, part_count, collapsed_chars}`.
