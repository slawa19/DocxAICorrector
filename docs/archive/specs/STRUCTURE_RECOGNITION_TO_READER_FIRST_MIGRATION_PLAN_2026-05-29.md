...
| PR-R0 | Translation Empty-Response Reliability | Completed locally as product-proof reliability unblocker: blank recovery output is converted into retryable `empty_response`, allowing the existing controlled source-block fallback after normal retry/recovery are exhausted. Proof `20260612T_pr_r0_empty_response_recovery_proof` completed the chapter-region comparison profile, produced openable DOCX, preserved images `12/12`, had no visible placeholder markup, and completed reader cleanup/verifier (`failed_chunk_count=0`, `cleaned_better`, high confidence). |
| PR-I2 | Formatting Preservation Implementation | Active: diagnostics enriched and first source-quality sub-slice completed locally. Heading-span merge proof `20260603T_pr_i2_heading_span_merge_proof` reduced `false_fragment_heading_count 15 -> 11`, worst `unmapped_source 56 -> 53`, worst `unmapped_target 48 -> 47`, and heading-role unmapped `12 -> 8`, with images `12/12` and cleanup safety flat. False-fragment gate now prefers entry-aware evidence in mixed fallback assemblies, and TOC/list entries aggregated into a mapped target are counted as covered. PR-R0 proof `20260612T_pr_r0_empty_response_recovery_proof` shows the profile now reaches product artifacts again and narrows the active blocker to formatting/mapping: acceptance failed only `formatting_diagnostics_threshold`, `unmapped_source_threshold`, and `unmapped_target_threshold` (`52` worst unmapped source vs threshold `12`, `48` target unmapped vs threshold `6`). Continue with style application across body/list TOC aggregation and merge/split shapes; do not repeat the reliability proof as the next step. |
