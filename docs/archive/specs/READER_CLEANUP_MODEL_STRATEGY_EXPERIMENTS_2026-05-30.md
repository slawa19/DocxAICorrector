...
| 2026-05-31 | `anthropic_small_overlap_pr_h0e_semantic_title_deletion_salience_proof` | `20260530_102025...raw.result.md` | 114894 chars / 311 blocks | `anthropic:claude-sonnet-4-6` | same | completed | Semantic-title deletion salience proof: 15 chunks, 0 failed chunks, 50 accepted operations, including 2 accepted side-heading reattach operations; broad unsafe `remove_inline_noise` proposals dropped from PR-H0d `1` to `0`; verifier `cleaned_better` high confidence, 19 remaining issues; semantic title is preserved but still needs a prefix-only/limitation decision | `.run/reader_cleanup_replay_experiments/20260531T151726Z_anthropic-small-overlap-pr-h0e-semantic-title-deletion-salience-proof/experiment_summary.md` |
| 2026-05-31 | `anthropic_small_overlap_pr_h0f_numeric_prefix_semantic_heading_proof_v3` | `20260530_102025...raw.result.md` | 114894 chars / 311 blocks | `anthropic:claude-sonnet-4-6` | same | completed with one-word policy decision remaining | Numeric-prefix semantic-heading proof: 15 chunks, 0 failed chunks, 56 accepted operations, including 24 `remove_inline_noise` and 3 `extract_side_heading_and_reattach_body`; broad unsafe `remove_inline_noise` proposals stayed `0`; verifier `cleaned_better` high confidence, 18 remaining issues; `20 ДЕНЬГИ, КОТОРЫЕ ПАХНУТ?` is cleaned to the preserved heading without numeric prefix, while `21 РОТТЕРДАМ.` remains a one-word heading policy decision | `.run/reader_cleanup_replay_experiments/20260531T162559Z_anthropic-small-overlap-pr-h0f-numeric-prefix-semantic-heading-proof-v3/experiment_summary.md` |
| 2026-06-01 | `gpt_5_4_mini_small_overlap_pr_h0h_control` | `20260530_102025...raw.result.md` | 114894 chars / 311 blocks | `gpt-5.4-mini` | `anthropic:claude-sonnet-4-6` | completed, not competitive | Same H0h payload/shape as Anthropic, judged by Anthropic verifier: 15 chunks, 0 failed chunks, 26 accepted operations, 59 remaining issues, `heading_fused_with_body=26`, `page_furniture_inline=7`, `fragmented_paragraph=16`, `prior_same_block_operation_not_applied=3`, broad unsafe `remove_inline_noise=0`; verdict still `cleaned_better` high confidence, but quality is far behind Anthropic H0h | `.run/reader_cleanup_replay_experiments/20260601T112649Z_gpt-5-4-mini-small-overlap-pr-h0h-control/experiment_summary.md` |
...
- Result after PR-H0e semantic-title deletion salience: `.run/reader_cleanup_replay_experiments/20260531T151726Z_anthropic-small-overlap-pr-h0e-semantic-title-deletion-salience-proof/`, `15` chunks, `0` failed chunks, `50` accepted operations, including `2` accepted `extract_side_heading_and_reattach_body` operations; verifier `cleaned_better` high confidence, `19` remaining issues. The broad unsafe `remove_inline_noise` proposal count is now `0`, so the model stopped proposing section-title-like text as noise. This is still not an MVP exit proof because the title line remains visible with a numeric prefix and needs a bounded prefix-only cleanup or product-limitation decision.
- Result after PR-H0f numeric-prefix semantic-heading cleanup: `.run/reader_cleanup_replay_experiments/20260531T162559Z_anthropic-small-overlap-pr-h0f-numeric-prefix-semantic-heading-proof-v3/`, `15` chunks, `0` failed chunks, `56` accepted operations, including `3` accepted `extract_side_heading_and_reattach_body` operations and `24` accepted `remove_inline_noise` operations; verifier `cleaned_better` high confidence, `18` remaining issues. The multi-word semantic title `20 ДЕНЬГИ, КОТОРЫЕ ПАХНУТ?` is now cleaned by removing only the exact numeric prefix while preserving the heading text, and broad unsafe `remove_inline_noise` stays `0`. This is still not an MVP exit proof because `21 РОТТЕРДАМ.` remains a one-word numeric-prefixed heading policy decision.
- Result after PR-H0g same-block join -> heading-boundary runtime contract: local tests on 2026-06-01 passed with `135` focused reader-cleanup tests, then replay `.run/reader_cleanup_replay_experiments/20260601T061315Z_anthropic-small-overlap-pr-h0g-same-block-join-heading-boundary-proof/` completed with `15` chunks, `0` failed chunks, `55` accepted operations, verifier `cleaned_better` high confidence, `17` remaining issues, `heading_fused_with_body=4`, `page_furniture_inline=1`, `prior_same_block_operation_not_applied=3`, and broad unsafe `remove_inline_noise=0`. This proves the runtime chain is available, but not that the model selects it for the important fused-heading anchors.
