...
- `DUP-003`: default image-processing summary shape is duplicated between `processing_runtime.py` and `state.py`.
...
- extract the currently duplicated response-output parsing helpers into one shared neutral module;
...
- text generation and image-related callers consume the same parser implementation;
...
- unify the default image-processing summary factory;
...
- image summary defaults stay next to session/runtime state semantics.

Acceptance:

- each shared shape/helper has one authoritative implementation;
...
- `app_runtime.py` manages worker/event/session bridging;
...
This is not fully broken, but boundaries are not explicit enough. That drives late imports, duplicated helper wiring, and review friction when changing a single flow.
...
3. `app_runtime.py`
   - owns event queues, worker start/stop bridging, and session-application of runtime events.
...
2. Removal of avoidable late imports introduced by fuzzy ownership.
3. Fewer top-level modules that need to be touched for a single orchestration change.
...
- prune or tighten registry/config fields that are technically present but not meaningfully variable today.
...
- align `app.py`, `application_flow.py`, `app_runtime.py`, `processing_service.py`, and `document_pipeline.py` with the target boundary contract;
...
3. `app.py`, `application_flow.py`, `app_runtime.py`, `processing_service.py`, and `document_pipeline.py` have a documented and visible ownership split.
...
6. Structural validation consumes a tighter production contract with less inline orchestration assembly.
