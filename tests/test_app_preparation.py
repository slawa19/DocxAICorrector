...
# --- A setting change must not silently destroy a delivered result -------------------
#
# ``start_background_preparation`` opens with an unconditional
# ``reset_run_state(keep_restart_source=False)``, so recomputing the preparation marker
# from the sidebar (target language / source language / text operation) used to wipe a
# finished, paid, possibly-undownloaded run with no prompt at all.

_PAID_SOURCE_TOKEN = "report.docx:3:ba7816bf8f01cfea"
_MARKER_BEFORE_TARGET_LANGUAGE_CHANGE = f"{_PAID_SOURCE_TOKEN}:6000:op=translate:sl=en:tl=ru"
_MARKER_AFTER_TARGET_LANGUAGE_CHANGE = f"{_PAID_SOURCE_TOKEN}:6000:op=translate:sl=en:tl=de"
_SIDEBAR_AFTER_TARGET_LANGUAGE_CHANGE = ("gpt-5.4", 6000, 3, "safe", False, "translate", "en", "de", False)


def _delivered_result(**overrides):
    result = {
        "source_name": "report.docx",
        "source_token": _PAID_SOURCE_TOKEN,
        "docx_bytes": b"paid-docx-bytes",
...
def _run_main_for_result_guard(
    monkeypatch,
    *,
    current_result: object = "from-session",
    sidebar_result=_SIDEBAR_AFTER_TARGET_LANGUAGE_CHANGE,
    stored_marker=_MARKER_BEFORE_TARGET_LANGUAGE_CHANGE,
    confirm_clicked=False,
    session_overrides=None,
):
    """Run ``main()`` for an upload whose preparation marker changed under the sidebar.
    ...
    uploaded_file = UploadedFileStub("report.docx", b"abc")
    record = {"starts": [], "warnings": [], "buttons": [], "rendered": [], "rerun": False}
    ...
    monkeypatch.setattr(
        app.st,
        "button",
        lambda label, *args, **kwargs: record["buttons"].append(label) or confirm_clicked,
    )
    monkeypatch.setattr(app, "render_intro_layout_styles", lambda: None)
    monkeypatch.setattr(app, "render_live_status", lambda *args, **kwargs: None)
    monkeypatch.setattr(app, "render_run_log", lambda *args, **kwargs: None)
...
def test_target_language_change_asks_before_destroying_the_delivered_result(monkeypatch):
    record = _run_main_for_result_guard(monkeypatch, current_result=_delivered_result())
    ...
    # The paid result stays on screen while the user decides, so the download is reachable.
    assert record["rendered"] == ["markdown_preview", "result_bundle"]
    assert record["rerun"] is False


def test_confirmed_discard_starts_the_preparation_for_the_new_setting(monkeypatch):
...
def test_first_run_of_a_file_starts_preparation_without_a_confirmation(monkeypatch):
    record = _run_main_for_result_guard(monkeypatch, current_result=None)
...
def test_uploading_a_different_document_starts_preparation_without_a_confirmation(monkeypatch):
    record = _run_main_for_result_guard(
        monkeypatch,
...
def test_setting_change_after_a_failed_run_starts_preparation_without_a_confirmation(monkeypatch):
    # No bundle stub here: the REAL get_current_result_bundle reads this session state and
    # returns None, which is what a run that failed without delivering anything looks like.
    record = _run_main_for_result_guard(
        monkeypatch,
        session_overrides={
            "processing_outcome": "failed",
...
def test_a_setting_outside_the_marker_neither_restarts_preparation_nor_asks_anything(monkeypatch):
    """The model selector is a real sidebar setting that is NOT part of the marker.

    Changing it destroys nothing, so it must cost no click at all: no preparation restart,
    no confirmation, and the delivered result still on screen.
    """
    prepared_run_context = _build_prepared_run_context(
    ...
    current_result = _delivered_result()
    record = {"starts": [], "warnings": [], "buttons": [], "rendered": []}

    monkeypatch.setattr(app.st, "session_state", session_state)
    monkeypatch.setattr(app, "init_session_state", lambda: None)
    monkeypatch.setattr(app, "_cached_load_app_config", lambda: {})
    # Same settings as the finished run except the model, which the marker ignores.
    monkeypatch.setattr(
        app,
        "render_sidebar",
        lambda config: ("gpt-5.4-mini", 6000, 3, "safe", False, "translate", "en", "de", False),
    )
    monkeypatch.setattr(app, "_drain_processing_events", lambda: None)
    monkeypatch.setattr(app, "_drain_preparation_events", lambda: None)
    monkeypatch.setattr(app, "_processing_worker_is_active", lambda: False)
    ...
    monkeypatch.setattr(app.st, "rerun", lambda: (_ for _ in ()).throw(AssertionError("no rerun expected")))
    monkeypatch.setattr(app, "render_intro_layout_styles", lambda: None)
    monkeypatch.setattr(app, "render_live_status", lambda *args, **kwargs: None)
    monkeypatch.setattr(app, "render_run_log", lambda *args, **kwargs: None)
    monkeypatch.setattr(app, "render_image_validation_summary", lambda *args, **kwargs: None)
    ...
    # Not vacuous: the marker really is unchanged, so the destructive branch is never entered.
    assert app.should_start_preparation_for_marker(_MARKER_AFTER_TARGET_LANGUAGE_CHANGE) is False
    assert record["starts"] == []
